        ...
        <div v-if="!!!suggestion" class="left">
          ...
          <div class="items">
            <SuggestionItem
              ...
              :key="suggestion.salle.nom"
              :item="suggestion.salle"
            />
          </div>
        </div>
        <div class="slot">
...
export default {
  ...
  methods: {
    fetchSuggestions() {
      axios
        .get(this.url + "/salle/suggestions/")
        .then((response) => {
          this.$store.state.suggestions = response.data;
          this.$store.state.images = response.data.map(
            (x) => x.salle.photo_principal
          );
        })
        .catch((error) => {
          console.error(error);
    ...
    search() {
      axios
        .get(this.url + `/salle/?search=${this.keyword}&date=${this.date}`)
        .then((response) => {
