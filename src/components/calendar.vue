...
export default {
  ...
  computed: {
    taken() {
      ...
      return !!salle
        ? this.$store.state.current_salle.taken.map((x) => x.date)
        : [];
    },
    month_name() {
    ...
    allocations: {
      get() {
        ...
        return !!this.salle.allocation
          ? this.salle.allocation.filter((x) => x.date.includes(month_str))
          : [];
      },
      set(new_val) {
  ...
  watch: {
    month(new_val) {
      ...
      let month = this.month <= 9 ? "0" + this.month : this.month;
      let month_str = `${this.year}-${month}`;
      this.allocations = this.salle.allocation.filter((x) =>
        x.date.includes(month_str)
      );
    },
  ...
  methods: {
    ...
    deleteRequest(id) {
      axios
        .delete(this.url + `/allocation/${id}/`, this.headers)
        .then((response) => {
    ...
    validateRequest(id) {
      axios
        .get(this.url + `/allocation/${id}/validate/`, this.headers)
        .then((response) => {
